...
//    *do not* mark the local address-exposed, instead marking the call with the "RETBUFFARG_LCLOPT"
//    flag and treating it like a store in liveness (and LIR interference checking). Note we only need
//    this special handling for the thin slice of compilation after lowering, since in HIR calls with
//    return buffers appear as ordinary struct-returning nodes.
//
